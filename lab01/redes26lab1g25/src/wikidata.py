...
def _request(params: dict) -> dict | None:
    ...
    params = {**params, "format": "json"} # crea un nuevo dict copiando params y forzando format=json
    try:
        resp = requests.get(
            WIKIDATA_BASE, # url
            params=params,
...
def _obtener_labels(qids: list[str]) -> dict[str, str]:
    ...
    for qid, ent in data.get("entities", {}).items():
        ...
        for lang in (LANG, "en"):
            if lang in labels:
                result[qid] = labels[lang].get("value", "")
                break
    return result
...
def _buscar_ids_wikidata(q: str) -> list[str] | None:
    ...
    return [i["id"] for i in search if "id" in i]
...
def _filtrar_ids_videojuegos(ids: list[str]) -> list[str] | None:
    ...
    return [qid for qid, ent in entities.items() if _es_videojuego(ent)]
...
def buscar(q: str) -> list[dict] | None:
    ...
    # 2. Filtrar solo videojuegos
    ids_videojuegos = _filtrar_ids_videojuegos(ids)
    if ids_videojuegos is None:
        return None
    if not ids_videojuegos:
        return []

    # 3. Obtener entidades completas
    entidades = _entidades_completas_videojuegos(ids_videojuegos)
    if entidades is None:
        return None

    # 4. Resolver labels de género y plataforma
    ref_qids = list(_colectar_ref_qids(entidades))
    labels_cache = _obtener_labels(ref_qids)

    # 5. Mapear, guardar y devolver
    return _mapear_y_guardar_resultados(ids_videojuegos, entidades, labels_cache)
...
def obtener_juego(juego_id: str) -> dict | None:
    ...
    """
    # Primero intentar desde catálogo local
    if juego_id in CATALOGO_JUEGOS:
        return CATALOGO_JUEGOS[juego_id]

    # Si no está, consultar Wikidata
    entidades = _entidades_completas_videojuegos([juego_id])
    if entidades is None or not entidades:
        return None

    entidad = entidades.get(juego_id, {})
    if not entidad or not _es_videojuego(entidad):
        return None

    # Resolver labels de género y plataforma
    ref_qids = list(_colectar_ref_qids({juego_id: entidad}))
    labels_cache = _obtener_labels(ref_qids)

    # Mapear, guardar y devolver
    juego = _mapear_entidad(entidad, labels_cache)
    CATALOGO_JUEGOS[juego_id] = juego
    _persist_catalogo()
    return juego
...
def listar_o_buscar_juegos():
    ...
    ordenar = request.args.get("ordenar", "id").strip().lower()
    orden = request.args.get("orden", "asc").strip().lower()

    lista = filtros.filtrar_y_ordenar(
        lista,
