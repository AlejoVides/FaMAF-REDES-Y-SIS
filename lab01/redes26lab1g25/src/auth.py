...
def registrar_usuario_auth():
    ...
    # la contraseña es hasheada usando werkzeug.
...
def login():
    ...
    # comparo la clave usando el hash.
    if not check_password_hash(cred["password_hash"], password):
        return jsonify({"error": "Credenciales inválidas"}), 401

    token = uuid4().hex
    vencimiento = (dt.datetime.now(dt.timezone.utc)+dt.timedelta(minutes=60)).isoformat().replace("+00:00", "Z")
    actualizar_token(cred["usuario_id"], token, vencimiento)
...
def obtener_usuario_actual() -> int | None:
    ...
    if not data.get("token_expira_en"):
        return None

    if dt.datetime.fromisoformat(data.get("token_expira_en").replace("Z", "+00:00")) <= dt.datetime.now(dt.timezone.utc):
        return None
