...
def listar_juegos_usuario(usuario_id: int):
    ...
    """
    for u in USUARIOS:
        enriquecidos = filter(None, map(_enriquecer_item, LISTAS_JUEGOS.get(usuario_id, [])))
        if u["id"] == usuario_id:
            return jsonify(filtros.filtrar_y_ordenar(
                enriquecidos,
...
def agregar_juego_usuario(usuario_id: int):
    ...
    if error:
        return error    # error 400 body invalido
    ...
    if existente:
        return jsonify({"error" : "El juego ya esta en la lista."}), 409
    ...
    enriquecido = _enriquecer_item(nuevo_item)

    return jsonify(enriquecido), 201
...
def actualizar_juego_usuario(usuario_id: int, juego_id: str):
    ...
    """
    lista, item_existente = _lista_y_existente(usuario_id, juego_id)

    if not lista:
        return jsonify({"error": "Usuario no existe"}), 404
    
    if not item_existente:
        return jsonify({"error": "El juego no es parte de la biblioteca"}), 404    
    
    data = request.json
    for key in ("tengo", "quiero", "jugado", "me_gusta"):
        if key in data:
            item_existente[key] = request.json[key]
    
    _persist_listas()
    enriquecido = _enriquecer_item(item_existente)
    return jsonify(enriquecido), 200
