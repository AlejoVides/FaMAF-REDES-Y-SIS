...
def sugerir_juego(usuario_id: int):
    ...
    if not candidatos:
        return jsonify({"error": "El usuario no tiene ningun juego"}), 404
    
    candidato = random.choice(candidatos)
    juego_elegido = CATALOGO_JUEGOS.get(candidato["juego_id"])

    sugerencia = {
        "id": juego_elegido.get("id", ""),
        "nombre": juego_elegido.get("nombre", ""),
        "descripcion": juego_elegido.get("descripcion", ""),
        "genero": juego_elegido.get("genero", ""),
        "lanzamiento": juego_elegido.get("lanzamiento", ""),
        "plataforma": juego_elegido.get("plataforma", ""),
    }
