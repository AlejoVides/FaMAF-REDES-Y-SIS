...
ax1.grid(axis='y')
fig1.tight_layout()
fig1.savefig('figuras/caso1_buffers_fuentes.png', dpi=300)
...
maximos = []
for _, row in buffers.iterrows():
    if 'node[1]' in row['module'] or 'node[6]' in row['module'] or 'node[7]' in row['module']:
        values = np.fromstring(row['vecvalue'], sep=' ')
        nodos.append(row['module'].replace('Network.', '').replace('.lnk[0]', ''))
        maximos.append(values.max())
ax2.bar(nodos, maximos)
ax2.set_xlabel('Nodo')
ax2.set_ylabel('Ocupación máxima (paquetes)')
fig2.tight_layout()
fig2.savefig('figuras/caso1_buffers_intermediarios.png', dpi=300)
...
ax3.grid(axis='y')
fig3.tight_layout()
fig3.savefig('figuras/caso1_delay.png', dpi=300)
...
ax4.grid(axis='y')
fig4.tight_layout()
fig4.savefig('figuras/caso1_hops.png', dpi=300)
...
ax3.grid(axis='y')
fig3.tight_layout()
fig3.savefig('figuras/caso1_link_utilization.png', dpi=300)
...
ax4.grid(axis='y')
fig4.tight_layout()
fig4.savefig('figuras/caso1_forwarded.png', dpi=300)
